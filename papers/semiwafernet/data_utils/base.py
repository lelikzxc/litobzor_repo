"""Base dataset class and dataset type enum for SemiWaferNet.

Provides ``BaseDataset`` (abstract base for all datasets) and ``DatasetType``
(enum for classification / segmentation / multitask).

This is a local replacement for ``common.datasets`` which may not be available.
"""
...
class BaseDataset(Dataset[dict[str, Any]]):
    ...
    Args:
        ...
        target_transform: Optional transform to apply to targets (masks).
    """
